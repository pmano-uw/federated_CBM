        ...
        for i in range(args['N']):
            ...
            try:
                with suppress_stdout_stderr():
                    fit = model.sample(data=dat, iter_sampling=2500, iter_warmup=1000, parallel_chains=4, 
                                        show_progress=False, seed=1234)
...
def device_posterior_update(r, Q, r_list, Q_list, diff_lk_list, model, args):
    ...
    for i in range(N):
        ...
        with suppress_stdout_stderr():
            fit_pred = model.sample(data=dat, iter_sampling=5000, iter_warmup=2500, parallel_chains=4, 
                                    adapt_delta=0.99, show_progress=False, seed=1234)
